# Creates the Solid Queue / Cache / Cable tables in the primary database.
#
# This app runs all four production connections (primary, cache, queue, cable) against a
# single Postgres. The Solid tables originally shipped as separate schema-dump files
# (db/{queue,cache,cable}_schema.rb) that `db:prepare` only loads when it *creates* a
# database. Because the production DB already existed, those schemas were never loaded and
# `solid_queue_jobs` et al. were missing. Defining them as a normal primary migration makes
# `db:migrate`/`db:prepare` create them on the existing DB and on any fresh one.
class CreateSolidQueueCacheCableTables < ActiveRecord::Migration[8.1]
  def up
    # ---- Solid Cache ----
